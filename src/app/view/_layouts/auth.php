...
<body style="background: url('<?= setting('site.login_cover') ?>') no-repeat center/cover; position: relative;">
    ...
    <div class="auth-left">
        ...
        <div style="position:absolute;bottom:0;left:0;width:100%;height:500px;background:linear-gradient(to top,rgba(52,58,64,0.9),rgba(52,58,64,0));z-index:2;"></div>
        <div class="deco-circle"></div>

        <div class="auth-left-inner">
            <div class="auth-tagline">
                <?php
                $quotes = setting('site.auth_quotes');
                if ($quotes) {
                    $quotesArray = array_filter(array_map('trim', explode("\n", $quotes)));
                    $quotesData = array_map(function ($line) {
                        $parts = explode(' - ', $line);
                        return [
                            'sentence' => $parts[0] ?? '',
                    ...
                    if (count($quotesData) > 0) {
                        $currentQuote = $quotesData[array_rand($quotesData)];
                        echo '<p>&quot;' . htmlspecialchars($currentQuote['sentence']) . '&quot;</p>';
                        echo '<h4>—' . htmlspecialchars($currentQuote['author']) . '</h4>';
                    } else {
                        echo '<p>&quot;Welcome to WebmanPanel!&quot;</p>';
                        echo '<h4>— The Team</h4>';
                    }
                }
                ?>
